...
export async function getServerSideProps() {
  const [operationsRes,sustainabilityRes,blogRes,whatsappRes] = await Promise.all([
    fetch(`https://whale-app-56hrz.ondigitalocean.app/api/sustainabilities/?populate=*`),
    fetch(`https://whale-app-56hrz.ondigitalocean.app/api/our-sustainability-activites/?populate=*`),
  ...
  const [operations,sustainability,blog,whatsapp] = await Promise.all([
    operationsRes.json(),
    sustainabilityRes.json(),
    blogRes.json(),
    whatsappRes.json()
  ]);
  return { props: { operations,sustainability,blog,whatsapp} };
}
export default function Home({operations,sustainability,blog,whatsapp}) {
  ...
  const onSubmit = async ({ full_name, company_name, email, contact_us, message }) => {
    ...
    if(res.data){
        alert("Form Saved Successfully.");
         router.push('https://grunerrenewable.com/sustainability');
...
{/* <div className="hide">1234567890</div> */}
  ...
  <div className="conatiner-fluid col-lg-12 col-md-12 " />
  {operations.data?.map( (row) => (
  <section id="Sustainability" className="d-flex align-items-center">
    <div className="container">
      <h1 className="pb-lg-3 sustainabilityheading">{row?.attributes.banner_title}</h1>
      <ReactMarkdown className="sustainability-about-paragraph">
...
{/* // </div> */}
{operations.data?.map( (row) => (
<div className="row g-0 responsivesomestatics">
    <div className=" col-12 col-6 col-md-4 responsivesome text-start p-lg-5">
      <span className="biospan p-lg-2">SOME STATICS</span>
      <h1 className="headingenergy my-3">{row?.attributes.some_statics_title}</h1>
      <ReactMarkdown className="biogaspara pt-4">
      {row?.attributes.some_statics_desription}
      </ReactMarkdown>
    </div>
      ...
      <div className="container text-center p-lg-5">
        <div className="row mx-auto">
          <div className="col-6 col-sm-3 ">
            <img src={ row?.attributes.some_statics_icon1.data[0].attributes.url} alt="" />
            <h1 className="sustainheader">{row?.attributes.some_statics_number1}+</h1>
            <ReactMarkdown className="sustainparagraph">
              {row?.attributes.some_statics_title1}
            </ReactMarkdown>
          </div>
          <div className="col-6 col-sm-3 ">
            <img src={ row?.attributes.some_statics_icon2.data[0].attributes.url} alt="" />
            <h1 className="sustainheader">{row?.attributes.some_statics_number2}+</h1>
            <ReactMarkdown className="sustainparagraph">
              {row?.attributes.some_statics_title2}
            </ReactMarkdown>
          </div>
          <div className="col-6 col-sm-3 ">
            <img src={ row?.attributes.some_statics_icon3.data[0].attributes.url} alt="" />
            <h1 className="sustainheader">{row?.attributes.some_statics_number3}</h1>
            <ReactMarkdown className="sustainparagraph">
              {row?.attributes.some_statics_title3}
            </ReactMarkdown>
          </div>
          <div className="col-6 col-sm-3 ">
            <img src={ row?.attributes.some_statics_icon4.data[0].attributes.url} alt="" />
            <h1 className="sustainheader">{row?.attributes.some_statics_number4}</h1>
            <ReactMarkdown className="sustainparagraph">
              {row?.attributes.some_statics_title4}
            </ReactMarkdown>
          </div>
        </div>
      </div>
...
{/* // </div> */}
  ...
  {/* media frame 3 end  here  */}
  {operations.data?.map( (row) => (
  <div className="container-fluid ">
    <div className="row ">
      <div className="col-lg-5 col-md-12 col-12 my-auto sustainabiltygreenfield">
        <div className="mission-main responsivesustain p-lg-5">
          <span className="mission p-lg-2">SUSTAINABLE DEVELOPMENT</span>
          <h4 className="pt-4">
          {row?.attributes.sustainable_development_title}
          </h4>
          <ReactMarkdown className="pt-lg-3">
          {row?.attributes.sustainable_development_desription}
          </ReactMarkdown>
        </div>
      </div>
      <div className="col-lg-7 col-12 px-0 text-end">
        <img src={ row?.attributes.sustainable_development_image.data[0].attributes.url} className="responsiveImageofMedia" alt="..." />
      </div>
    </div>
  </div>
    ))}
  {/*OUR SUSTAINABILITY GOALS  start from here */}
  {operations.data?.map( (row) => (
  <div className="container-fluid ">
    <div className="row">
      <div className="col-lg-6 col-12 px-0 text-start">
        <img src={ row?.attributes.our_goals_image.data[0].attributes.url}  className="imageindex responsiveImageofMedia marginofphotos" alt="..." />
      </div>
      <div className="col-lg-6 col-12 my-auto sustainabiltygreenfieldsecond">
        <div className="vision-main paddingfromleft">
          <div className='  pt-5'>
          <span className="mission  responsivegole p-lg-2">OUR GOALS</span>
          </div>
          <h1 className='pt-4 spacemarigin'>{row?.attributes.our_goals_title}</h1>
          <ReactMarkdown className="pt-lg-3 loremsustaibility">
          {row?.attributes.our_goals_desription}
          </ReactMarkdown>
          <div className="media">
            <img
              className="mr-3 smallpicture"
              src={ row?.attributes.our_goals_icon1.data[0].attributes.url}
              alt="Generic placeholder image"
              
            />
            <div className="media-body">
              <h5 className="mt-0 sustainheading">{row?.attributes.our_goals_title1}</h5>
              <ReactMarkdown className='loremsustaibility'> 
              {row?.attributes.our_goals_desription1}
              </ReactMarkdown>
            </div>
          </div>
          <div className="media">
            <img
              className="mr-3 smallpicture"
              src={ row?.attributes.our_goals_icon2.data[0].attributes.url}
              alt="Generic placeholder image"
            />
            <div className="media-body">
              <h5 className="mt-0 sustainheading">{row?.attributes.our_goals_title2}</h5>
              <ReactMarkdown className='loremsustaibility'> 
              {row?.attributes.our_goals_desription2}
              </ReactMarkdown>
            </div>
          </div>
          <div className="media">
            <img
              className="mr-3 smallpicture"
              src={ row?.attributes.our_goals_icon3.data[0].attributes.url}
              alt="Generic placeholder image"
            />
            <div className="media-body">
              <h5 className="mt-0 sustainheading">{row?.attributes.our_goals_title3}</h5>
              <ReactMarkdown className='loremsustaibility'> 
              {row?.attributes.our_goals_desription3}
              </ReactMarkdown>
            </div>
          </div>
          <div className="media">
            <img
              className="mr-3 smallpicture"
              src={ row?.attributes.our_goals_icon4.data[0].attributes.url}
              alt="Generic placeholder image"
            />
            <div className="media-body">
              <h5 className="mt-0 sustainheading">{row?.attributes.our_goals_title4}</h5>
              <ReactMarkdown className='loremsustaibility'> 
              {row?.attributes.our_goals_desription4}
              </ReactMarkdown>
            </div>
          </div>
        </div>
  ...
  {/*OUR SUSTAINABILITY GOALS  start from here */}
  {sustainability.data?.map( (row) => (
  <div className="container-fluid p-0">
    <section className=" hero p-lg-5">
      <div className="row p-lg-5">
        <div className="col-12 col-lg-6 p-lg-4 startingpadding">
          <span className="Biocng responsiveour p-lg-2">OUR SUSTAINABILITY ACTIVITIES</span>
          <h1 className="biocngheading resposnveheadingsust pt-4">{row?.attributes.activities_title}</h1>
          <ReactMarkdown className="paraheadingcng pt-3">
          {row?.attributes.activities_desription}
          </ReactMarkdown>
        </div>
        <div className="col-12 col-lg-6">
        <img src="../images/sustcrowser.png" alt="" />
        </div>
      </div>
    </section>
  </div>
   ))}
  <div className=" conatiner-fluid col-12 col-lg-12  pt-5">
    <span className="biospan mx-5 p-lg-2 blogsspan">Blogs</span>
    <div class="row g-0">
  <div class="col-sm-6 col-12 col-md-9"><h2 className='pt-3 mediaparagraphheading mx-5'>Latest Articles For Our Clients </h2></div>
  <div class="col-6  col-12 col-md-3">
   <a href='/blogs'>
   <button className='sustainbuttonblue'>View All <i class="bi bi-arrow-right"></i> </button>
   </a>
  </div>
</div>
  </div>
    ...
    <div className=" col-12 col-lg-6">
      <div className="card">
        ...
        <div className="card-body pt-3 pb-4">
          <h5 className="card-title">{row?.attributes.title}</h5>
          <ReactMarkdown className="card-text">
          {row?.attributes.description}
          </ReactMarkdown>
         <a href={`/blogs/${row?.attributes.Slug}`}> <button className='buttonsustain'>
            Read More
            <i className="bi bi-arrow-right-short mx-3" />
          </button></a>
        </div>
      </div>
    </div>
