...
export async function getServerSideProps() {
  const [operationsRes,logoRes,teamRes] = await Promise.all([
    ...
    fetch(`https://whale-app-56hrz.ondigitalocean.app/api/about-teams/?populate=*`)

    // fetch(`http://localhost:1337/api/services/?populate=*`),
    // fetch(`http://localhost:1337/api/careers/?populate=*`)
  ]);
  const [operations,logos,team] = await Promise.all([
    operationsRes.json(),
    logoRes.json(),
    teamRes.json()
  ]);
  return { props: { operations,logos,team} };
}

export default function Home({operations,logos,team}) {
  return (
    <>
      <Head>
