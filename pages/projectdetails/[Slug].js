...
// let courseimage = [];
export const Slug = ({product,programs}) => {
    ...
    programs.data.map(row=>{
      if(row.attributes.Slug == query.Slug){
          ...
          row.attributes.project_image.data?.map(image=>{
 ...
 <div className="container-fluid" id="flex-conatiner1">
    <div className="row ">
      <div className="col-12 col-lg-3 my-auto text-center">
        <img src="https://gremedia.sgp1.digitaloceanspaces.com/media/logo-gre.svg" className="img-fluid" id="logo" alt="Gruner Renewable Energy" href="/index"/>
      </div>
...
<i className="fa fa-whatsapp my-float"></i>
  ...
  {courseimage.map( (row) => (
    <div className="">
      <img src={ row?.attributes.url} alt className='projectdeatilsFrameImages'/>
    </div>
     ))} 
    </div>
    {/* over-view start here */}
    <div className=" text-start mb-2 projectdetailsfrompage">
      <div className="row">
        <div className="col-12 col-lg-8 mt-5 ">
          <span className="overviewspan p-lg-2">OVERVIEW</span>
          <h3 className="headingoverview my-3">  {product.attributes.description}</h3>
          <ReactMarkdown className="overviewpara">
          {product.attributes.detail}
          </ReactMarkdown>
        </div>
        {/* over view end here */}
        {/*greenblankfoam start from here */}
        <div className="col-sm-12 col-lg-4 project-details-from">
          <div className="row g-3 align-items-start my-5">
            <div className="col-auto">
              <label htmlFor="Client" className="placeholderdetails">Client</label>
            </div>
            <div className="col-auto">
            <p className='clientdetails'>   {product.attributes.client}</p>
            </div>
          </div> 
          <div className="row g-3 align-items-center my-auto">
            <div className="col-auto">
              <label htmlFor="Client" className="placeholderdetails">Location :</label>
            </div>
            <div className="col-auto">
            <p className='mt-4 clientdetails'>   {product.attributes.location}</p>
            </div>
          </div> 
          <div className="row g-3 align-items-center my-2">
            <div className="col-auto">
              <label htmlFor="Client" className="placeholderdetails">Year :</label>
            </div>
            <div className="col-auto">
            <p className='mt-2 clientdetails'>   {product.attributes.year}</p>
            </div>
          </div> 
          <div className="row g-3 align-items-center my-5">
            <div className="col-auto">
              <label htmlFor="Client" className="placeholderdetails">Project Size :</label>
            </div>
            <div className="col-auto">
            <p className='mt-2 clientdetails'>   {product.attributes.project_size}</p>
            </div>
          </div> 
          <div className="row g-3 align-items-center my-2">
            <div className="col-auto">
              <label htmlFor="Client" className="placeholderdetails">Highlight :</label>
            </div>
            <div className="col-auto">
              <p className='mt-4 clientdetails'>   {product.attributes.highlight}</p>
            </div>
          </div> 
        </div>  
        {/*greenblankfoam end from here  */}      
      </div>
...
export async function getServerSideProps(context) {
    const [productRes,programsRes] = await Promise.all([
        fetch(`https://whale-app-56hrz.ondigitalocean.app/api/project-details/?filters[Slug]=` + context.query.Slug ),
        fetch(`https://whale-app-56hrz.ondigitalocean.app/api/project-details/?populate=*`)

    ]);
    const [product,programs] = await Promise.all([
        productRes.json(),
        programsRes.json()
    ]);
    return { props: {product:product.data[0],programs} };
}
export default Slug